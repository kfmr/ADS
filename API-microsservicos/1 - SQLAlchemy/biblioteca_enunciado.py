...
def todos_alunos():
    with engine.connect() as con:  # conectar ao banco de dados
        ...
        executeQuery = con.execute(sql_consulta).fetchall()
        result = []
        for item in executeQuery:
...
def all_books():
    with engine.connect() as con:
        query_sql = "Select * from livro"

        execute_query = con.execute(query_sql).fetchall()
        result = []
...
def create_book(id_livro, id_aluno, descricao):
    with engine.connect() as con:
        query_sql = text(
            "INSERT INTO Livro (id_livro, id_aluno, descricao) VALUES (:id_livro, :id_aluno, :descricao)")

        execute_query = con.execute(
            query_sql, id_livro=id_livro, id_aluno=id_aluno, descricao=descricao)


#create_book(4, 2, "API em nodejs")
...
def empresta_livro(id_livro, id_aluno):
    with engine.connect() as con:
        ...
        execute_query = con.execute(
            query_sql, id_livro=id_livro, id_aluno=id_aluno)
...
def devolve_livro(id_livro):
    with engine.connect() as con:
        query_sql = text(
            "Update Livro SET id_aluno = :id_aluno where id_livro = :id_livro")
        execute_query = con.execute(
            query_sql, id_aluno="", id_livro=id_livro)
...
def livros_parados():
    with engine.connect() as con:
        query_sql = text(
            "select * from livro where id_aluno = :id_aluno")
        execute_query = con.execute(
            query_sql, id_aluno="").fetchall()
...
def livros_do_aluno(id):
    with engine.connect() as con:
        query_sql = text("select * from livro where id_aluno = :id_aluno")

        execute_query = con.execute(
            query_sql, id_aluno=id).fetchall()

        return execute_query


# print(livros_do_aluno(2))
